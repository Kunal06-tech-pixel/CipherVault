...
const config = z.object({
  ...
  databaseUrl: secret('DATABASE_URL') ?? 'postgres://ciphervault:ciphervault@localhost:5432/ciphervault',
...
const emailWorker = new Worker('ciphervault-email', async (job) => {
  ...
  try {
    await client.sendAsync({
      ...
      to: payload.email,
      subject: job.name === 'recovery' ? 'Recover your CipherVault account' : 'Verify your CipherVault account',
      text: job.name === 'recovery' ? `Continue recovery within 30 minutes: ${payload.url}\n\nYour offline recovery key is also required.` : `Verify your CipherVault account within 30 minutes: ${payload.url}`,
      attachment: [{ data: job.name === 'recovery' ? `<p>Continue recovery within 30 minutes.</p><p><a href="${payload.url}">Recover account</a></p><p>Your offline recovery key is also required.</p>` : `<p>Verify your CipherVault account within 30 minutes.</p><p><a href="${payload.url}">Verify account</a></p>`, alternative: true }],
    })
  } finally {
    client.smtp.close()
  }
...
console.log('CipherVault worker started with redacted operational logging.')
