...
export function registerSessionRoutes(app: FastifyInstance, context: ApiContext): void {
  ...
  app.get('/v1/account', { preHandler: requireAuthentication }, async (request) => ({
    id: request.auth!.user.id,
    email: request.auth!.user.email,
    emailVerified: Boolean(request.auth!.user.emailVerifiedAt),
  }))

  app.get('/v1/sessions', { preHandler: requireAuthentication }, async (request) =>
    database.listSessions(request.auth!.user.id, request.auth!.session.id))

  app.delete('/v1/sessions/:id', { preHandler: requireAuthentication }, async (request, reply) => {
  ...
  app.delete('/v1/account', { preHandler: requireAuthentication }, async (request, reply) => {
