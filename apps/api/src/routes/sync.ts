...
export function registerSyncRoutes(app: FastifyInstance, { database }: ApiContext): void {
  const syncRateLimit = { max: 120, timeWindow: '1 minute' }

  app.get('/v1/sync', { preHandler: requireAuthentication, config: { rateLimit: syncRateLimit } }, async (request, reply) => {
  ...
  app.post('/v1/sync', { preHandler: requireAuthentication, config: { rateLimit: syncRateLimit } }, async (request, reply) => {
