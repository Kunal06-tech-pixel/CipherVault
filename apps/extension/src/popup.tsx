...
function App() {
  ...
  if (!status.unlocked) return <main className="popup locked"><div className="brand"><span><ShieldCheck size={19} /></span><b>CipherVault</b></div><div className="lock-art"><LockKeyhole size={27} /></div><h1>Extension locked</h1><p>Unlock the web vault, then pair this browser to enable private autofill.</p><button onClick={() => chrome.tabs.create({ url: `${WEB_URL}/?extension=pair` })}><KeyRound size={16} /> Open CipherVault</button><small>Keys are cleared after 5 minutes of inactivity.</small></main>

  return <main className="popup"><div className="brand"><span><ShieldCheck size={19} /></span><b>CipherVault</b><i>Unlocked</i></div><div className="search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search this site" /></div>
